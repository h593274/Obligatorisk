...
public class GPSComputer {
	...
	public double totalDistance() {
		...
		for (int i = 0; i < gpspoints.length; i++) {
			...
			avstand = distance(gpspoints[i + 1], gpspoints[i]);
		}
	...
	public double maxSpeed() {
		...
		for (int i = 0; i <= gpspoints.length; i++) {
			maxspeed = gpspoints[i].getTime() * distance(gpspoints[i + 1], gpspoints[i]);
		}
	...
	public double kcal(double weight, int secs, double speed) {
		...
		if (speedmph < 10) {
			...
			met = 10;
		} else if (speedmph > 16 && 18 > speedmph) {
			met = 12;
		} else {
			met = 16;
		}
	...
	public double totalKcal(double weight) {
		...
		for (int i = 0; i <= gpspoints.length; i++) {
			totalkcal += kcal(weight, gpspoints[i].getTime(), GPSUtils.speed(gpspoints[i + 1], gpspoints[i]));
		}
		return totalkcal;
	}
